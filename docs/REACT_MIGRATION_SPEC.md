...
- **Tło:** gradient (`body`) + radial accent — `--nx-app-bg` / `--nx-app-bg-accent`; **nieco głębsze** niż płaski szary, żeby karty i shell nie „zlewały się” z tłem.
- **Karty / powierzchnie:** klasa **`nx-product-surface`** — border-radius **~14px**; `--nx-surface-elevated` / `--nx-surface-inset` do separacji tonów.
...
- **Sidenav:** szeroki panel po lewej, brand **„Nexir Core Workspace”** (**title-medium**), linia oddzielająca, lista: Dashboard, Entities, Favorites, Settings; na dole **Account**. Linki nawigacji: zaokrąglenie, krótki **transition** tła, **focus-visible** dla klawiatury; aktywny link: delikatne tło z mix primary + przezroczystość (jak `shell-nav-item--active`).
...
- Nagłówek: back **„Entities”** / **„Back”**, tytuł strony, podtytuł.
- **Stopka akcji:** **Cancel** po **lewej**; **Generate sample data** + **Create** / **Save changes** po **prawej** (grupa).
...
2. **`formatCatalogLabel` / `entityStatusBadgeTone`** — `entity-catalog-display.ts` (etykiety komórek i mapowanie statusu → ton chipa).
3. **Generowanie id** `ent-NNN` od max istniejących numerów.
4. **Password rules** — identyczne regexy i komunikaty.
5. **Sample payload** — `getSampleEntityPayload()` stały obiekt spójny semantycznie.
...
- Hero: pojedynczy **H1** **Nexir Core Workspace** (headline-small, kolor primary), podtytuł o neutralnej powłoce (bez drugiego tytułu „Workspace”).
...
- Intro w karcie: tytuł **Application map** (headline-small), podtytuł, przycisk **Back to our view** → `/`.
- Listy tras i bloki **Code layout** (`dl`) jako **kafelki** (obramowanie, zaokrąglenie, lekki inset).
...
- **Status:** chipy (pill) z mapowaniem `entityActive` / `entityDraft` / `entityArchived`; etykieta **title case** (`formatCatalogLabel`); **gwiazdka** ulubionych w kolorze **bursztynowym** (`#f59e0b`).
